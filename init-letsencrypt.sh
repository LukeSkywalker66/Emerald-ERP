...
if [ -d "$data_path" ]; then
  read -p "Ya existen datos de certificados. ¿Quieres reemplazarlos? (y/N) " decision
...
if [ ! -e "$data_path/conf/options-ssl-nginx.conf" ] || [ ! -e "$data_path/conf/ssl-dhparams.pem" ]; then
  echo "### Descargando parámetros de seguridad recomendados..."
...
# Seleccionar modo staging o producción
case "$staging" in
