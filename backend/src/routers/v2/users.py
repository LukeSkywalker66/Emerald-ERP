...
def reset_password_admin(
    ...
    # 🔒 AUDIT LOG: Registrar reseteo de contraseña
    try:
        log_update(
            db=db,
            ...
            new_values={
                "password_reset": True,
                "method": "custom" if payload and payload.new_password else "autogenerated",
                "ip_address": get_client_ip(request) if request else None,
            }
        )
    except Exception as audit_error:
        logger.error(f"❌ [AUDIT] Error al registrar reseteo de contraseña del usuario {user.id}: {audit_error}")
