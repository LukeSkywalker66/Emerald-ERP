...
def main():
    ...
    try:
        # Obtener todos los eventos de tipo STATUS_CHANGE (mayúsculas)
        result = db.execute(
            text("SELECT id, content FROM ticket_timeline WHERE event_type IN ('STATUS_CHANGE', 'status_change') ORDER BY id")
        ).fetchall()
        
        print(f"\n📝 Procesando {len(result)} eventos de timeline...\n")
        updated = 0
        ...
        for event_id, content in result:
            ...
            if new_content != content:
                ...
                updated += 1
                print(f"  ✓ [{event_id:3d}] {content}")
                print(f"        → {new_content}\n")
        
        db.commit()
        print(f"✅ Actualizados {updated} de {len(result)} eventos a español\n")
